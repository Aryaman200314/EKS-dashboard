...
def get_security_insights(cluster_raw, eks_client):
    ...
    insights['logging_enabled'] = {
        "status": all(lt in enabled_logs for lt in all_logs),
...
def get_live_eks_data():
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        ...
        for account in accounts_to_scan:
            ...
            if not session:
                errors.append(f"Failed session for account {account['id']}.")
                continue
        ...
        for future in as_completed(describe_futures):
            ...
            try:
                ...
                errors.append(f"Error describing cluster: {e}")

    processed_clusters = [_process_cluster_data(c) for c in all_clusters_raw]
    for c in processed_clusters:
        c['createdAt'] = c['createdAt'].isoformat() if isinstance(c.get('createdAt'), datetime) else c.get('createdAt')
